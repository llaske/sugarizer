...
define(["webL10n"], function (l10n) {
	...
	util.timestampToElapsedString = function(timestamp, maxlevel) {
	 ...
	 // Convert click to tap
	 util.clickToTap = function(object, sender, event) {
		// HACK: On IE, onclick = ontap
		if (enyo.platform.ie || (!enyo.platform.safari && !enyo.platform.firefox && !enyo.platform.chrome && !enyo.platform.android) || enyo.platform.android) {
			enyo.bind(object, sender.ontap)(sender, event);
		}
	 }
	 
	 // Get client type
	 util.getClientType = function() {
		return document.location.protocol.substr(0,4) == "http" ? constant.thinClientType : constant.clientType;
	 }
	 util.getClientName = function() {
		return this.getClientType() == constant.thinClientType ? "Thin Client" : "Client";
	 }
