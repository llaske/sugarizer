...
define(["webL10n","sugar-web/datastore","FileSaver"], function (l10n, datastore, FileSaver) {
	...
	util.writeFile = function(directory, metadata, content, callback) {
		...
		if (metadata && metadata.mimetype) {
			...
			if (mimetype == "image/jpeg") {
				...
				extension = "docx";
			} else if (mimetype == "application/vnd.ms-word;charset=utf-8") {
				extension = "doc";
			} else {
				extension = "bin";
			}
	...
	function writeFileToStore(file, text, callback) {
		if (file.type == 'application/json') {
			...
			var activity = "";
			if (file.type != "text/plain" && file.type != "application/pdf" && file.type != "application/vnd.openxmlformats-officedocument.wordprocessingml.document" && file.type != "application/vnd.ms-word;charset=utf-8") {
	...
	util.loadFile = function(file, callback) {
		...
		if (util.getClientType() == constant.webAppType || constant.noServerMode || (util.getClientType() == constant.appType && !enyo.platform.android && !enyo.platform.androidChrome && !enyo.platform.ios && !enyo.platform.electron)) {
			...
			var validTypes = ['application/json','image/jpeg','image/png','audio/wav','video/webm','audio/mp3','audio/mpeg','video/mp4','text/plain','application/pdf','application/vnd.openxmlformats-officedocument.wordprocessingml.document','application/vnd.ms-word;charset=utf-8'];
