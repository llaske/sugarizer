...
define(["l10n","sugar-web/datastore","FileSaver"], function (l10n, datastore, FileSaver) {
	...
	util.timestampToElapsedString = function(timestamp, maxlevel, issmall, ago_string="Ago") {
		...
		if (levels == 0) {
			return l10n.get("Seconds"+ago_string+suffix);
		}

		return l10n.get(ago_string+suffix, {time:time_period});
	}
	...
	// Scan code functions
	// Doesn't work until fix https://github.com/mebjas/html5-qrcode/issues/856
	util.scanQRCode = function(okCallback, cancelCallback) {
		...
		var showQRDialog = function() {
			document.getElementById("toolbar").style.visibility = "hidden";
			document.getElementById("qrbackground").style.visibility = "visible";
			...
			document.getElementById("qrclosebutton").addEventListener('click', onButtonClose);
			document.getElementById("qrswitchbutton").addEventListener('click', onButtonSwitch);
		}

		var hideQRDialog = function() {
			...
			document.getElementById("qrclosebutton").removeEventListener('click', onButtonClose);
			document.getElementById("qrswitchbutton").removeEventListener('click', onButtonSwitch);
			document.getElementById("toolbar").style.visibility = "visible";
			document.getElementById("qrbackground").style.visibility = "hidden";
		}

		var onScanSuccess = function(decodedText, decodedResult) {
			html5Qrcode.stop();
			hideQRDialog();
			okCallback(decodedText);
		}
		...
		var startScan = function() {
			showQRDialog();
			html5Qrcode = new Html5Qrcode("qrbackground");
			html5Qrcode.start(
	...
	function writeFileToStore(file, text, callback) {
		if (file.type == 'application/json') {
			...
			if (file.type == "application/vnd.ms-excel" || file.type == "text/csv" || file.type == "text/x-csv" || file.type == "text/tab-separated-values" || file.type == "text/comma-separated-values") {
				activity = "org.sugarlabs.ChartActivity";
				mimetype = "text/csv";
			} else if (file.type != "text/plain" && file.type != "application/pdf" && file.type != "application/msword" && file.type != "application/vnd.oasis.opendocument.text") {
	...
	util.loadFile = function(file, callback) {
		...
		if (util.getClientType() == constant.webAppType || constant.noServerMode || util.platform.android || util.platform.ios || (util.getClientType() == constant.appType && !util.platform.electron)) {
			...
			var validCsvTypes = ['application/vnd.ms-excel', 'text/csv', 'text/x-csv', 'text/tab-separated-values', 'text/comma-separated-values'];	
			if (validTypes.indexOf(file.type) == -1 && validCsvTypes.indexOf(file.type) == -1) {
				callback(file.name, -1);
				return;
			}
			...
			if (file.type == 'application/json' || file.type == 'text/plain' || (validCsvTypes.includes(file.type))) {
