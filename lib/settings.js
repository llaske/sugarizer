...
define(["sugar-web/datastore"], function (datastore) {
	...
	settings.load = function(then) {
		...
		datastore.localStorage.load(function() {
			...
			if (preferences.language !== undefined) that.language = preferences.language;
			if (preferences.connected !== undefined) that.connected = preferences.connected;
			if (preferences.server !== undefined) that.server = preferences.server;
			if (preferences.networkId !== undefined) that.networkId = preferences.networkId;
	...
	// Activity handling
    settings.runActivity = function(activity, objectId, name, sharedId) {
	if (activity.type && activity.type == "Android"){
	    if (sugarizerOS){
