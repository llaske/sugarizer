	...
	tutorial.init = function() {
		...
		tour = new Tour({
			...
			onShown: function() {
				...
				if (app.getView() == constant.radialView && tour.getCurrentStep()==0) {
					// HACK: If network performance are poor, tutorial must be restarted on the home view
					window.setTimeout(function() {
						var popuptuto = document.getElementsByClassName("popover tour-tour");
						if (popuptuto.length == 0) {
							tour.end();
							window.setTimeout(function() {
								app.getToolbar().startTutorial();
							}, constant.timerBeforeTutorial*2)
						}
					}, constant.timerBeforeTutorial*2);
				}
			},
			...
			var steps = [
				[
					...
					},
					{
						...
						content: l10n.get("TutoInitStopContent")
					},
				],
				[
					{
