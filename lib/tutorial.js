...
define([], function () {
	...
	tutorial.init = function() {
		...
		tour = new Tour({
			...
			steps: [],
			onShown: function() {
				if (tutorial.icon && tutorial.icon.steps && tutorial.icon.steps.indexOf(tour.getCurrentStep()) != -1) {
					var icon = document.getElementById("icon-tutorial");
					icon.style.visibility = 'visible';
					icon.style.backgroundImage = "url('icons/" + tutorial.icon.icon + "')";
					icon.style.backgroundSize = tutorial.icon.size + "px";
					icon.style.width = tutorial.icon.size + "px";
					icon.style.height = tutorial.icon.size + "px";
					icon.style.marginTop = "15px";
					icon.style.marginLeft = "5px";
					iconLib.colorize(icon, tutorial.icon.color, function(){});
				}
			},
			onEnd: function() {
				tutorial.elements = [];
				...
				[
					{
						...
						placement: "bottom",
						title: (app.isCreatingNewUser() ? l10n.get("TutoInitNameNewTitle") : l10n.get("TutoInitNameTitle")),
						content: (app.isCreatingNewUser() ? l10n.get("TutoInitNameNewContent") : l10n.get("TutoInitNameContent"))
					},
					previous,
					next,
				],
				[
					{
						...
						placement: "right",
						title: (app.isCreatingNewUser() ? l10n.get("TutoInitPasswordNewTitle") : l10n.get("TutoInitPasswordTitle")),
						content: (app.isCreatingNewUser() ? l10n.get("TutoInitPasswordNewContent") : l10n.get("TutoInitPasswordContent"))
					},
					previous,
					next,
				],
				[
					{
						element: tutorial.getElement("owner"),
						placement: "right",
						title: l10n.get("TutoInitColorTitle"),
						content: l10n.get("TutoInitColorContent")
					},
					previous,
					next,
				],
			];
			tour.addSteps(steps[app.getStep()]);
			if (app.getStep() == 0) {
				tutorial.icon = {icon: 'owner-icon.svg', size: constant.sizeEmpty, color: {stroke: "#005FE4", fill: "#FF2B34"}, steps: [0]};
			}
		}
		tour.init();
	}
