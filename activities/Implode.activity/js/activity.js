			  ...
			  for(var j=max_size[1]-1;j>=0;j--){
				...
				square.addEventListener('mouseover', function(e){
					...
					|| (e.target.highlighted == false))
				  {
					...
					var square_color = stage.getChildByName(`${square_name[0]}_${square_name[1]}`).graphics._instructions[2].style
				  ...
				  for (var row = 0; row < max_size[1]; row++) {
					...
					var upper_block = stage.getChildByName(`${col}_${index + 1}`);
							  ...
							  if (index < (max_size[1] - 1)) {
								  target_block.graphics._instructions[2].style = upper_block.graphics._instructions[2].style;
		  ...
		  function shiftLeft(){
			...
			for (var col = 0; col < max_size[0]; col++) {
				...
				for(var col_index=col;col_index<max_size[0];col_index++){
				  ...
				  {
					while(stage.getChildByName(`${col_index + k}_0`).graphics._instructions[2].style == backgorund_color){
					  k++;
					  if((col_index + k) == max_size[0]){
						break;
					  }
				  ...
				  for(var row=0;row < max_size[1];row++){
					  ...
					  if((col_index + k) != max_size[0]){
						...
						current.graphics._instructions[2].style = block_on_right.graphics._instructions[2].style;
						stage.getChildByName(`${col_index + k}_${row}`).graphics._instructions[2].style = backgorund_color;
					  }
				  }
				}
			  }
