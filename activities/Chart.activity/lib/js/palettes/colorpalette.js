...
define([
    ...
    function hexToRgb(hex) {
        // Expand shortForm (e.g. "03F") to fullForm (e.g. "0033FF")
    ...
    function parseColor(input) {
        if (input.trim().startsWith("#")) input = hexToRgb(input);        
    ...
    function initGui() {
        ...
        colorPaletteFill.addEventListener('color-change', onColorChangeFill);
        var colorInvokerFill = colorPaletteFill.getPalette().querySelector('.palette-invoker');
        colorPaletteFill.setColor(0);

        colorPaletteStroke.addEventListener('color-change', onColorChangeStroke);
