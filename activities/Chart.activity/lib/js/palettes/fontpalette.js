...
define([
  ...
  textpalette.TextPalette = function(invoker, primaryText) {
    ...
    this.fontChangeEvent.initCustomEvent('font-change', true, true, {
	  'family': 'Arial'
    });
    this.template = '<center><table><tbody>' + '{{#rows}}' + '<tr>' + '{{#.}}' + '<td style="padding:5px">' + '<button lineHeight="{{lineHeight}}" title="{{fontFamily}}" style="height:55px; width:55px; background-size:40px !important;  background: #fff url({{ icon }}) no-repeat center; "></button>' + '</td>' + '{{/.}}' + '</tr>' + '{{/rows}}' + '</tbody></table></center>';
    var rows = [
      [{
