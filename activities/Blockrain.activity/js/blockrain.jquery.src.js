  ...
  $.widget("aerolab.blockrain", {
    ...
    _checkCollisions: function (x, y, blocks, checkDownOnly) {
      ...
      for (; i < len; i += 2) {
        ...
        if (b < 0) {
          continue; // Allow checking other blocks instead of stopping early
        }
        ...
        if (!checkDownOnly && (a < 0 || a >= this._BLOCK_WIDTH)) {
          return true;
        }
      }
      return false;
...
,
    ...
    _createUI: function () {
      ...
      game._$highscore = $(
        '<div class="blockrain-Hscore-holder" style="position:absolute;">' +
          ...
          '<div class="blockrain-Hscore-msg">' +
    ...
    _setupTouchControls: function (enable) {
      ...
      $("#left-arrow").bind("touchstart click", function (event) {
        moveLeft(event);
        endMoveLeft(event);
      });
      $("#right-arrow").bind("touchstart click", function (event) {
        moveRight(event);
        endMoveRight(event);
      });
      $("#up-arrow").bind("touchstart click", rotateRight);
      $("#down-arrow").bind("touchstart click", function (event) {
        game.start();
        drop(event);
        endDrop(event);
      });
