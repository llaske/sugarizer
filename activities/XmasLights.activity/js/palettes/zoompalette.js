...
define(['sugar-web/graphics/palette', 'mustache'], function(palette, mustache) {
	...
	zoompalette.ZoomPalette = function(invoker, primaryText) {
		...
		this.zoomEvent.initCustomEvent('zoom', true, true, {'zoom': 3});
		...
		var rows = [
			[{icon: 'icons/zoom-out.svg'}, {icon: 'icons/zoom-in.svg'}, {icon: 'icons/zoom-original.svg'}]
		];
		var zoomElem = document.createElement('div');
		zoomElem.innerHTML = mustache.render(this.template, {rows: rows});
